...
function renderArena(){
  const p = currentPair();
  if (!p){
// 顯示最終排名：切換 finished 狀態，讓整頁可捲動
    const t = $("#tournament"); if (t) t.classList.add("finished");
    ...
    // 進行中：移除 finished 狀態（恢復一頁對戰的版型）
  const t = $("#tournament"); if (t) t.classList.remove("finished");
